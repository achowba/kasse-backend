...
- Each test covers three things: the happy path, the failure the code is written to reject, and the edge case that breaks a naive implementation. For this project the edge cases that matter are an empty range, a plan of zero, absent spend, a boundary month, a negative amount, and a malformed CSV row.
...
- Coverage is a floor, not a goal. A pure calculation like the variance module is expected to be covered exhaustively, including every branch of the plan-is-zero and missing-spend cases. Wiring code is not padded with tests to raise a number.
